...
<div style="display:flex;align-items:center;gap:1rem;margin-bottom:2rem;">
  ...
  <div style="flex:1;">
    <h1 style="font-size:1.375rem;font-weight:800;color:#1E293B;margin:0 0 .1rem;letter-spacing:-.02em;">Edit Layanan</h1>
    <p style="font-size:.8rem;color:#94A3B8;margin:0;">Perbarui informasi dan konten: <strong style="color:#3B82F6;">{{ $s->name }}</strong></p>
  </div>
  <a href="{{ url('/services/'.$s->slug) }}" target="_blank" style="display:inline-flex;align-items:center;gap:.375rem;font-size:.8rem;font-weight:700;color:#8B5CF6;background:rgba(139,92,246,0.08);padding:.5rem 1rem;border-radius:10px;text-decoration:none;transition:all .2s;" onmouseover="this.style.background='rgba(139,92,246,0.15)'" onmouseout="this.style.background='rgba(139,92,246,0.08)'">
...
<div style="display:flex;flex-direction:column;gap:1.5rem;">
  ...
  <div style="background:#fff;border-radius:20px;padding:1.75rem;box-shadow:0 2px 20px rgba(0,0,0,0.04);">
    ...
    <div id="faq-container" style="display:flex;flex-direction:column;gap:.875rem;">
      ...
      @empty
      <div style="text-align:center;padding:2rem;color:#94A3B8;font-size:.875rem;border:2px dashed #E4E7F0;border-radius:10px;">Belum ada FAQ. Klik "Tambah FAQ" untuk mulai.</div>
      @endforelse
    </div>
  </div>
...
<div style="display:flex;flex-direction:column;gap:1.5rem;position:sticky;top:1.5rem;">
    ...
    <div style="display:flex;flex-direction:column;gap:1rem;">
      <label style="display:flex;align-items:center;justify-content:space-between;cursor:pointer;padding:.875rem 1rem;background:#F8FAFC;border-radius:12px;border:1.5px solid #E4E7F0;">
        ...
        <div style="position:relative;">
          ...
          <input type="checkbox" name="is_active" value="1" {{ old('is_active',$s->is_active)?'checked':'' }} id="is_active_toggle" style="position:absolute;opacity:0;width:0;height:0;" onchange="updateToggle(this)">
          <div id="toggle-track" onclick="document.getElementById('is_active_toggle').click()" style="width:44px;height:24px;border-radius:100px;cursor:pointer;transition:background .2s;position:relative;background:{{ old('is_active',$s->is_active)?'#3B82F6':'#E4E7F0' }};">
            <div id="toggle-thumb" style="position:absolute;top:3px;left:{{ old('is_active',$s->is_active)?'23px':'3px' }};width:18px;height:18px;background:#fff;border-radius:50%;transition:left .2s;box-shadow:0 1px 4px rgba(0,0,0,0.15);"></div>
          </div>
        </div>
      </label>
      <div>
  ...
  <div style="background:#fff;border-radius:20px;padding:1.5rem;box-shadow:0 2px 20px rgba(0,0,0,0.04);">
    ...
    @if($s->brochure)
    <a href="{{ asset('storage/'.$s->brochure) }}" target="_blank" style="display:inline-flex;align-items:center;gap:.375rem;font-size:.78rem;font-weight:700;color:#3B82F6;background:rgba(59,130,246,0.08);padding:.4rem .875rem;border-radius:8px;text-decoration:none;margin-bottom:.875rem;transition:background .2s;" onmouseover="this.style.background='rgba(59,130,246,0.15)'" onmouseout="this.style.background='rgba(59,130,246,0.08)'">
      <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
    ...
    <label style="display:flex;flex-direction:column;align-items:center;gap:.5rem;padding:1.25rem;border:2px dashed #E4E7F0;border-radius:12px;cursor:pointer;transition:all .2s;text-align:center;" onmouseover="this.style.borderColor='#3B82F6';this.style.background='#F8FAFF'" onmouseout="this.style.borderColor='#E4E7F0';this.style.background='transparent'">
      ...
      <span style="font-size:.8rem;color:#64748B;font-weight:600;">{{ $s->brochure ? 'Ganti Brosur' : 'Upload Brosur' }}</span>
