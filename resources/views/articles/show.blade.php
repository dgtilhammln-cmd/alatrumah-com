...
@section('content')
    ...
    ═══════════════════════════════════════ */
        ...
        *::after {
        ...
        .ar-related-card:hover .ar-related-card-img img {
            transform: scale(1.07);
        }

        .ar-related-card-body {
            padding: 1.25rem;
        }
    ...
    <section class="sv-hero-premium">
        <div class="sv-hero-inner">
            ...
            @if(isset($article->authorRel) && $article->authorRel->slug)
                <a href="{{ url("/{$locale}/author/" . $article->authorRel->slug) }}" class="ar-author-row"
                    style="text-decoration:none; display:inline-flex; transition:all .2s; padding-right:1rem; border-radius:12px; margin-left:-0.5rem; padding-left:0.5rem; border-top:none; margin-top:1.5rem; padding-top:0.5rem; border:1px solid transparent;"
                    onmouseover="this.style.background='rgba(14,165,233,0.05)';this.style.borderColor='rgba(14,165,233,0.1)'"
                ...
                @endif
                    <div class="ar-author-avatar" style="background:transparent; width:42px; height:42px; overflow:hidden;">
                        @if(isset($article->authorRel) && $article->authorRel->getRawOriginal('photo'))
                            <img src="{{ asset('storage/' . $article->authorRel->getRawOriginal('photo')) }}"
                                alt="{{ $article->authorRel->name }}" style="width:100%; height:100%; object-fit:cover;">
                        @elseif(!empty($settings['logo']))
                            <img src="{{ asset('storage/' . $settings['logo']) }}" alt="Alat Rumah"
                                style="width:100%; height:100%; object-fit:contain;">
                        @else
                            <span
                                style="background:var(--c-accent); width:100%; height:100%; display:flex; align-items:center; justify-content:center; color:#fff; font-weight:800; font-size:1rem; border-radius:50%;">C</span>
                        @endif
                    </div>
                    <div>
                        <div class="ar-author-name"
                            style="transition:color .2s; {{ isset($article->authorRel) && $article->authorRel->slug ? 'color:var(--c-accent);' : '' }}">
                            {{ isset($article->authorRel) ? $article->authorRel->name : ($article->author ?? 'Tim Alat Rumah') }}
                        </div>
    ...
    <section class="ar-body-section">
        ...
        @if($article->show_toc && count($article->toc) > 0)
            <details class="ar-toc" open>
                <summary class="ar-toc-header" style="list-style:none; outline:none;">
                    ...
                    <svg style="color:var(--c-accent); flex-shrink:0; transition:transform 0.3s;" width="16" height="16"
        ...
        <article class="ar-content" dir="{{ $locale === 'ar' ? 'rtl' : 'ltr' }}">
            @if(!empty($trans?->featured_snippet))
                <div class="featured-snippet-box"
                    style="padding:1.5rem; background:#F8FAFC; border-left:4px solid var(--c-accent); border-radius:8px; margin-bottom:2rem;">
                    <strong
                        style="color:var(--c-accent); font-size:0.85rem; text-transform:uppercase; letter-spacing:0.05em; display:block; margin-bottom:0.5rem;">{{ $locale === 'ar' ? 'ملخص سريع' : ($locale === 'en' ? 'Quick Summary' : ($locale === 'ko' ? '요약' : 'Ringkasan Cepat')) }}</strong>
                    <span
                        style="font-size:1.05rem; line-height:1.6; color:#0F172A; font-weight:500;">{{ $trans->featured_snippet }}</span>
                </div>
            @endif
            @php
        ...
        @if(isset($article->authorRel))
            ...
            @if($authorTrans && !empty($authorTrans->bio))
                <div class="ar-author-bio-box"
                    style="margin-top:3.5rem; padding:2rem; background:#ffffff; border:1px solid #E2E8F0; border-radius:16px; display:flex; gap:1.5rem; align-items:flex-start; box-shadow:0 10px 25px rgba(0,0,0,0.02);"
                    ...
                    <div style="width:72px; height:72px; border-radius:50%; overflow:hidden; flex-shrink:0; background:#F1F5F9;">
                        ...
                        @else
                            <span
                                style="width:100%; height:100%; display:flex; align-items:center; justify-content:center; font-size:1.5rem; font-weight:700; color:#94A3B8;">{{ substr($article->authorRel->name, 0, 1) }}</span>
                        @endif
                    </div>
                    <div>
                        <h4 style="margin:0 0 0.5rem; font-size:1.125rem; font-weight:700; color:#0F172A; font-family:var(--font);">
                            @if($article->authorRel->slug)
                                <a href="{{ url("/{$locale}/author/" . $article->authorRel->slug) }}"
                                    style="color:inherit; transition:color .2s;" onmouseover="this.style.color='var(--c-accent)'"
                                    onmouseout="this.style.color='inherit'">{{ $article->authorRel->name }}</a>
                            @else
                                {{ $article->authorRel->name }}
                            @endif
                        </h4>
                        <p style="margin:0; font-size:0.95rem; color:#475569; line-height:1.6; font-family:var(--font);">
                            {{ $authorTrans->bio }}</p>
                                ...
                                @foreach($article->authorRel->social_links as $sname => $surl)
                                    @if(!empty($surl))
                                        <a href="{{ $surl }}" target="_blank" rel="noopener noreferrer"
                                            style="font-size:0.8rem; font-weight:600; color:var(--c-accent); text-decoration:none;">{{ ucfirst($sname) }}</a>
                                    @endif
                                @endforeach
                            </div>
                        @endif
        ...
        <div
            style="position:relative; overflow:hidden; background:linear-gradient(135deg, #ffffff 0%, #F8FAFC 100%); border:1px solid #E2E8F0; padding:2rem 2.5rem; margin:2.5rem 0; border-radius:16px; box-shadow:0 12px 32px rgba(15,23,42,0.04); display:flex; flex-direction:column; gap:1.25rem;">
            <div
                style="position:absolute; top:-50px; right:-50px; width:150px; height:150px; background:radial-gradient(circle, rgba(14,165,233,0.1) 0%, transparent 70%); border-radius:50%; pointer-events:none;">
            </div>
            <div
                style="position:absolute; bottom:-30px; right:10%; width:100px; height:100px; background:radial-gradient(circle, rgba(14,165,233,0.05) 0%, transparent 70%); border-radius:50%; pointer-events:none;">
            </div>

            <div style="display:flex; align-items:center; gap:1rem;">
                <div
                    style="width:42px; height:42px; border-radius:12px; background:rgba(14,165,233,0.08); color:#0EA5E9; display:flex; align-items:center; justify-content:center; flex-shrink:0;">
                    <svg width="22" height="22" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" />
                    </svg>
                </div>
                <h4 style="margin:0; font-size:1.15rem; color:#0F172A; font-weight:700; letter-spacing:-0.01em;">Lengkapi
                    Kebutuhan Rumah Anda</h4>
            </div>

            <p style="margin:0; color:#475569; font-size:0.95rem; line-height:1.65; max-width:95%;">Tingkatkan kenyamanan
                rumah dengan produk dari AlatRumah.com. Kualitas terjamin, harga bersahabat, dan bergaransi resmi.</p>

            <a href="{{ url('/produk') }}"
                style="align-self:flex-start; margin-top:0.25rem; display:inline-flex; align-items:center; gap:0.5rem; background:#0EA5E9; color:#fff; padding:0.75rem 1.75rem; border-radius:50px; font-weight:600; text-decoration:none; font-size:0.925rem; transition:all 0.3s; box-shadow:0 4px 12px rgba(14,165,233,0.2);"
                onmouseover="this.style.background='#0284C7'; this.style.transform='translateY(-2px)'; this.style.boxShadow='0 8px 20px rgba(14,165,233,0.3)';"
                onmouseout="this.style.background='#0EA5E9'; this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 12px rgba(14,165,233,0.2)';">
                    ...
                    @foreach($articleFaqs as $faq)
                        @if(!empty($faq['q']))
                            <details class="ar-faq-item">
                                <summary>
                                    {{ $faq['q'] }}
                                    <svg class="ar-faq-chevron" width="16" height="16" fill="none" stroke="currentColor"
        ...
        @if($articleCta && !empty($articleCta['text']))
            ...
            <div class="ar-cta-box">
                ...
                <p>Temukan ribuan produk alat rumah tangga berkualitas dengan harga terbaik, dikirim ke seluruh Indonesia dari
                    Surabaya.</p>
                <a href="{{ route_locale('products') }}" class="ar-cta-btn"
        ...
        <div class="ar-share">
            ...
            <a href="https://wa.me/?text={{ $shareTitle }}%20{{ $shareUrl }}" target="_blank" rel="noopener"
                class="ar-share-btn"
                style="color:#25D366;background:rgba(37,211,102,0.08);border-color:rgba(37,211,102,0.25);">
            ...
            <a href="https://www.facebook.com/sharer/sharer.php?u={{ $shareUrl }}" target="_blank" rel="noopener"
                class="ar-share-btn"
                style="color:#1877F2;background:rgba(24,119,242,0.08);border-color:rgba(24,119,242,0.25);">
                <svg width="15" height="15" fill="currentColor" viewBox="0 0 24 24">
            ...
            <a href="https://twitter.com/intent/tweet?url={{ $shareUrl }}&text={{ $shareTitle }}" target="_blank"
                rel="noopener" class="ar-share-btn"
                style="color:#000;background:rgba(0,0,0,0.05);border-color:rgba(0,0,0,0.12);">
                <svg width="14" height="14" fill="currentColor" viewBox="0 0 24 24">
                    <path
                        d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.746l7.73-8.835L1.254 2.25H8.08l4.259 5.632zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                </svg>
                X
            </a>
            ...
            <a href="https://www.linkedin.com/sharing/share-offsite/?url={{ $shareUrl }}" target="_blank" rel="noopener"
                class="ar-share-btn"
                style="color:#0A66C2;background:rgba(10,102,194,0.08);border-color:rgba(10,102,194,0.25);">
                <svg width="15" height="15" fill="currentColor" viewBox="0 0 24 24">
            ...
            <a href="https://t.me/share/url?url={{ $shareUrl }}&text={{ $shareTitle }}" target="_blank" rel="noopener"
                class="ar-share-btn"
                style="color:#229ED9;background:rgba(34,158,217,0.08);border-color:rgba(34,158,217,0.25);">
                <svg width="15" height="15" fill="currentColor" viewBox="0 0 24 24">
                    <path
            ...
            {{-- Copy Link --}}
            <button id="copyBtn" class="ar-share-btn"
                style="color:var(--c-muted);background:var(--c-surface);border-color:var(--c-border); cursor:pointer;"
    ...
    <style>
        .cv-section-label {
            ...
            text-transform:uppercase; color:var(--c-muted); margin-bottom:.875rem;
        }
        ...
        .cv-section-title {
            ...
            font-weight:500; color:var(--c-text); line-height:1.15;
            letter-spacing:-.03em; margin-bottom:0;
        }
        .sh-adv-section {
            background:var(--c-bg); padding:5rem 1.5rem;
            border-top:1px solid var(--c-border);
        }
        ...
        .sh-adv-header-desc { max-width:320px; font-size:.875rem; color:var(--c-muted); line-height:1.65; text-align:right; }
        .sh-adv-cards {
        ...
        .sh-adv-card:hover { border-color:var(--c-accent); transform:translateY(-6px); box-shadow:0 16px 40px rgba(14,165,233,.08); }
        .sh-adv-card.accent { background:var(--c-accent); border-color:var(--c-accent); }
        .sh-adv-card.accent-dark { background:var(--c-text); border-color:var(--c-text); }
        .sh-adv-card-icon {
            width:44px; height:44px; border-radius:12px;
            display:flex; align-items:center; justify-content:center; flex-shrink:0;
        }
        .sh-adv-card-icon.blue-bg { background:#E0F2FE; color:var(--c-accent); }
        ...
        .sh-adv-num { font-size:2.5rem; font-weight:300; line-height:1; letter-spacing:-.04em; color:var(--c-text); }
        .sh-adv-num.white { color:#fff; }
        .sh-adv-num.blue { color:#38BDF8; }
        .sh-adv-title { font-size:.9375rem; font-weight:600; color:var(--c-text); }
        .sh-adv-title.white { color:#fff; }
        .sh-adv-title.light { color:rgba(255,255,255,.9); }
        .sh-adv-desc { font-size:.8rem; color:var(--c-muted); line-height:1.6; margin-top:auto; }
        ...
        .sh-coverage-title {
            ...
            font-weight: 500; color:var(--c-text); line-height:1.1;
        ...
        .sh-stat-label { font-size:.65rem; font-weight:600; color:var(--c-muted); text-transform:uppercase; letter-spacing:.1em; }
        .sh-stat-icon { color:var(--c-text); opacity:.8; }
        .sh-stat-val { font-size:3.5rem; font-weight:300; color:var(--c-text); line-height:1; letter-spacing:-.05em; display:flex; align-items:baseline; gap:0.1em; }
        .sh-stat-val span { color:var(--c-accent); font-size:2rem; font-weight:600; line-height:1; }
        
        @media (max-width: 1024px) {
            .sh-adv-cards { grid-template-columns: repeat(2, 1fr); }
        ...
        @media (max-width:640px) {
            .sh-adv-section, .sh-coverage-section { padding:3.5rem 1rem; }
            .sh-adv-cards { grid-template-columns:1fr; }
                    ...
                    @foreach($related as $r)
                        <a href="{{ route_locale('articles.show', $r->slug) }}" class="ar-related-card" data-aos="fade-up">
                            <div class="ar-related-card-img">
                                ...
                                @else
                                    <div
                                        style="width:100%;height:100%;min-height:160px;background:linear-gradient(135deg,#E2E8F0,#CBD5E1);display:flex;align-items:center;justify-content:center;">
